...
class ErrorHandlerTest {
  ...
  def should_not_parse_error {
    try {
    	obj.throwException(StatusCode(400, ""))
    	fail
    } catch {
  ...
  def should_not_parse_error_trying_oauth {
    try {
    	obj.throwOAuthException(StatusCode(400, ""))
    	fail
    } catch {
      case e: CreataryException => assertThat(e.response, is(unknownError))
      case _ => fail
  ...
  def should_parse_error_trying_oauth {
    try {
    	obj.throwOAuthException(StatusCode(400, """{"error":"invalid_request"}"""))
    	fail
    } catch {
  ...
  def should_forward_parsing_error {
    try {
    	obj.throwException(StatusCode(400, """{"error":"invalid_request"}"""))
    	fail
    } catch {
      case e: OAuthException => assertThat(e.error, is("invalid_request"))
      case e => fail
  ...
  def should_parse_error {
    try {
    	obj.throwException(StatusCode(400, "{\"status\":{\"code\":\"100\",\"message\":\"invalid_request\"}}"))
    	fail
    } catch {
